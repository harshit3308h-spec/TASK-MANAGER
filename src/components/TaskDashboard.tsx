...
import { CheckCircle2, Circle, Play, Pause, Plus, Trash2, ArrowUpDown, Edit2, Save, X, Sparkles, Clock, AlertTriangle, Timer, Mic } from 'lucide-react';
...
export default function TaskDashboard({ onAddExp, tasks }: TaskDashboardProps) {
  ...
  const handleCompleteTask = async (id: string, e?: React.MouseEvent) => {
    ...
    if (isMounted.current) {
        setFlash(true);
        setTimeout(() => { if (isMounted.current) setFlash(false); }, 300);
    }

    setFinishingTaskIds(prev => [...prev, id]);

    // Delay for animation, then update DB
    setTimeout(async () => {
        onAddExp(earnedExp);
        ...
        await db.tasks.update(id, {
            ...
            isRunning: false,
            lastUpdated: undefined, // Clear timer
  ...
  const confirmDelete = async () => {
    if (taskToDelete) {
        await db.tasks.update(taskToDelete, {
            deleted: true,
            completedAt: new Date().toISOString(),
            isRunning: false,
  ...
  const addTask = async (title: string) => {
    ...
    const newTask: Task = {
      id: self.crypto.randomUUID(), // Production-ready ID generation
      ...
      reminder: newTaskDeadline || undefined
    };
  ...
  const toggleTimer = async (id: string, e?: React.MouseEvent) => {
    if (e) e.stopPropagation();
    const task = tasks.find(t => t.id === id);
    if (!task) return;

    const isStarting = !task.isRunning;
    const now = Date.now();
  ...
  const saveEdit = async () => {
      ...
      const isReminderChanged = task?.reminder !== editForm.reminder;
      ...
      await db.tasks.update(editingTaskId, {
        ...
        reminderFired: isReminderChanged ? false : task?.reminderFired
      });
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto relative">
      ...
      <div className={`fixed inset-0 bg-white/10 pointer-events-none transition-opacity duration-300 ease-out z-[100] ${flash ? 'opacity-100' : 'opacity-0'}`} />
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[150] flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-zinc-900 border border-white/10 p-6 rounded-xl max-w-sm w-full shadow-2xl animate-in zoom-in-95 duration-200">
                ...
                <p className="text-zinc-400 mb-6 text-sm leading-relaxed">
                    Are you sure you want to abandon this task? It will be marked as failed.
                </p>
                <div className="flex justify-end gap-3">
                    <button onClick={() => setTaskToDelete(null)} className="px-4 py-2 rounded-lg text-zinc-400 hover:bg-white/5 hover:text-white transition-colors text-xs font-bold uppercase tracking-wider">Cancel</button>
                    <button onClick={confirmDelete} className="px-4 py-2 rounded-lg bg-red-600 hover:bg-red-500 text-white text-xs font-bold uppercase tracking-wider shadow-[0_0_15px_rgba(220,38,38,0.4)] transition-all">Abandon Quest</button>
                </div>
            </div>
        </div>
      )}
      ...
      <div className="flex justify-between items-center mb-4 px-1">
        ...
        <div className="flex items-center gap-2 bg-zinc-900/30 backdrop-blur-sm px-3 py-1.5 rounded-lg border border-white/10 hover:border-white/20 transition-colors group">
            ...
            >
                <option value="newest" className="bg-zinc-900">Default</option>
              ...
              return (
                <div key={task.id} className="bg-black/60 backdrop-blur-md p-4 rounded-xl border border-indigo-500/50 shadow-2xl flex flex-col gap-3 animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <div className="flex justify-end items-center pt-2 gap-2">
                        <button onClick={cancelEdit} className="flex items-center gap-1 px-3 py-1.5 text-zinc-400 hover:text-white hover:bg-white/5 rounded-lg text-xs font-bold transition-all"><X size={14} /> Cancel</button>
                        <button onClick={saveEdit} className="flex items-center gap-1 px-4 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-xs font-bold transition-all shadow-[0_0_15px_rgba(99,102,241,0.4)]"><Save size={14} /> Save</button>
                    </div>
                </div>
              );
          }
          ...
          return (
            ...
            >
                    ...
                    <>
                        ...
                        <div className="absolute inset-0 flex items-center justify-center z-30 pointer-events-none">
                             <div className="transform scale-150 animate-[bounce_0.8s_infinite]">
                                <span className={`text-5xl font-black text-transparent bg-clip-text drop-shadow-[0_0_15px_rgba(251,191,36,0.6)] ${isOverdue ? 'bg-gradient-to-b from-red-300 to-red-500' : 'bg-gradient-to-b from-yellow-300 to-amber-500'}`}>
                                    +{isOverdue ? Math.floor(task.exp / 2) : task.exp} XP
                                </span>
                             </div>
                        </div>
                        <div className="absolute top-2 right-10 z-20 text-yellow-300 animate-[spin_1s_ease-out_forwards]"><Sparkles size={32} /></div>
                        <div className="absolute bottom-2 left-10 z-20 text-emerald-300 animate-[pulse_0.5s_ease-in-out_infinite]"><Sparkles size={24} /></div>
                    </>
                )}
                ...
                <div className="flex items-start z-20">
                    ...
                    className={`mt-1 mr-4 transition-colors ${isFinishing ? 'cursor-default text-emerald-500' : isOverdue ? 'text-red-500 hover:text-red-400' : 'text-zinc-600 hover:text-emerald-400'}`}
                    ...
                    <div className="flex-1 min-w-0 mr-4 group cursor-pointer" onDoubleClick={() => startEditing(task)}>
                        ...
                        <div className="flex gap-4 mt-2 items-center">
                            {task.voiceNote && <span className="text-xs text-indigo-400 flex items-center"><Mic size={10} className="mr-1"/> Transcribed</span>}
                            {task.deadline && (
                            ...
                            <span className={`text-xs flex items-center font-mono transition-all duration-300 ${task.isRunning ? 'text-emerald-300 bg-emerald-500/20 px-2 py-0.5 rounded border border-emerald-500/30 shadow-[0_0_10px_rgba(16,185,129,0.2)]' : 'text-zinc-500'}`}>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                        <span className={`text-xs font-bold px-2 py-1 rounded hidden sm:inline-block transition-colors ${isFinishing ? 'text-emerald-300 bg-emerald-500/20 opacity-0' : isOverdue ? 'text-red-400 bg-red-500/10 border border-red-500/20' : 'text-amber-500 bg-amber-500/10 border border-amber-500/20'}`}>
                            +{isOverdue ? Math.floor(task.exp / 2) : task.exp} XP
                        </span>
                        <button onClick={(e) => { e.stopPropagation(); startEditing(task); }} className="p-2 text-zinc-600 hover:text-indigo-400 transition-colors disabled:opacity-0" title="Edit Quest" disabled={isFinishing}><Edit2 size={16} /></button>
                        ...
                        <button onClick={(e) => initiateDelete(task.id, e)} className="p-2 text-zinc-600 hover:text-red-400 transition-colors disabled:opacity-0" title="Abandon Quest" disabled={isFinishing}><Trash2 size={16} /></button>
                    </div>
                </div>
            </div>
          );
